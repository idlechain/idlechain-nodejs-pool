	...
	switch (message.type) {
		...
		case 'BlockTemplate':
			...
			//	}
				...
				log('error', logSystem, `BlockTemplateiiiiii ${e}`);
			}
			break;
...
/*	let template_hex = EXTRA_NONCE_TEMPLATE;
	...
	if (!mergedMining && this.isRandomX) {
		...
		this.next_seed_hash = template.seed_hash; //template.next_seed_hash;
	}
	/*
...
function Miner (rewardType, childRewardType, id, childPoolIndex, login, pass, ip, port, agent, childLogin, startingDiff, noRetarget, pushMessage) {
	...
	this.extranonce = generateExtraNonce();
...
Miner.prototype = {
	...
	getJob: function () {
		...
		let blob = blockTemplate.blocktemplate_blob;
		blob = this.extranonce + blob.substring(4);
...
function validateMinerAddress(str) {
    // Verifica si la cadena tiene longitud 42 y comienza con "0x"
...
function handleMinerMethod (method, params, ip, portData, sendReply, pushMessage) {
	...
	switch (method) {
		case 'login':
			...
			if (validateMinerAddress(address) == false) {
				let addressPrefix = 'N/A';
		...
		case 'submit':
			...
			if (!noncePattern.test(params.nonce)) {
				let minerText = miner ? (' ' + miner + '@' + miner.ip) : '';
...
*/
			let blockTemplate = currentBlockTemplate[miner.activeChildPool];
			let shareAccepted = true;
			shareAccepted = processShare(miner, job, blockTemplate, params);
...
function processShare (miner, job, blockTemplate, params) {
	...
	if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability) {
	    ...
	    let convertedBlob = blockTemplate.blocktemplate_blob; //utils.cnUtil.convert_blob(shareBuffer, cnBlobType);
		convertedBlob = convertedBlob.substring(0, 78) + params.nonce + convertedBlob.substring(86);
		convertedBlob = miner.extranonce + convertedBlob.substring(4);
		let hard_fork_version = 1; //convertedBlob[0];

		//if (blockTemplate.isRandomX) {
			hash = cryptoNight(Buffer.from(convertedBlob, 'hex'), Buffer.from(blockTemplate.seed_hash, 'hex'), cnVariant);
			log('info', logSystem, hash.toString('hex') + " / " + resultHash);
		//} else {
		//	if (typeof config.includeHeight !== "undefined" && config.includeHeight)
	...
	//recordShareData(miner, job, hashDiff.toString(), false, hash.toString('hex'), shareType, blockTemplate, null);
	...
	if (hashDiff.ge(blockTemplate.difficulty)) {
		...
		apiInterfaces.rpcDaemon('submitblock', [miner.extranonce + params.nonce], function (error, result) {
			if (error) {
				...
				log('info', logSystem, 
					...
					[result.hash.substr(0, 6), job.height, miner.login, miner.ip]
				);
				recordShareData(miner, job, hashDiff.toString(), true, result.hash, 'valid', blockTemplate, null);
			}
		});
